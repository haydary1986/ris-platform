...
async function fetchCollege(slug: string) {
...
async function fetchResearchers(collegeId: string) {
  const supabase = await createClient();
...
export default async function CollegePage({ params }: CollegePageProps) {
  ...
  const typedLocale = locale as Locale;
  const collegeName = typedLocale === 'ar' ? college.name_ar : college.name_en;
  ...
  return (
    <main className="container mx-auto flex flex-col gap-6 px-4 py-8">
      ...
      <h1 className="text-3xl font-semibold tracking-tight">{collegeName}</h1>
